...
WrappingBufferView::WrappingBufferView(const string &name,i64 x,i64 y,i64 w,i64 h)
		:drawx(x),drawy(y),draww(w),drawh(h),name(name){
	buffer.addCursor(0,0);
}

WrappingBufferView::~WrappingBufferView(){}

void WrappingBufferView::setText(const string &text){
	buffer.setText(text);
}

string WrappingBufferView::fullText() const {
		...
		switch(dir){
			case FindDir::forward:
				if(scrolly==buffer.numLines()-1){
					cerr<<"[BV] WARNING: FindDir::forward at scrolly=="<<scrolly
			...
			case FindDir::backward:
				if(scrolly==0){
					cerr<<"[BV] WARNING: FindDir::backward at scrolly==0"<<endl;
					return {scrolly,layout};
				}
				scrolly--;
				break;
	...
	if(viewReposition){
		...
		pair<int,TextLayout::Layout> found=findLayout(
			buffer,draww-gutterWidth,drawh,scrolly,[targety](const TextLayout::Layout &layout) -> FindDir {
				if(layout.lines.size()==0){
					cerr<<"[BV] Predicate: layout.lines.size() == 0"<<endl;
					return FindDir::backward;
				}
				cerr<<"[BV] Predicate: targety="<<targety<<" l.l[0].fLN = "<<layout.lines[0].fromLineNum<<endl;
...
void WrappingBufferView::draw(i64 x,i64 y,i64 w,i64 h){
	drawx=x;
	drawy=y;
	draww=w;
	drawh=h;
...
void WrappingBufferView::setName(const string &newname){
	name=newname;
}

const string& WrappingBufferView::getName() const {
	return name;
}

bool WrappingBufferView::handleKey(int key){
	switch(key){
		case KEY_BACKSPACE:
